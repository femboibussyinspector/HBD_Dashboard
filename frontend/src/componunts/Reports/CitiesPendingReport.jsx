...
export function CitiesPendingReport() {
  ...
  // 1. Get List of Unique Categories (for Dropdown)
  const uniqueCategories = useMemo(() => {
    return [...new Set(listingData.map((item) => item.category))].filter(Boolean).sort();
  }, []);

  // 2. Get List of Unique Cities (Rows)
  ...
  const tableRows = useMemo(() => {
    ...
    return cities.map(city => {
      // Calculate pending count for this City AND selected Category
    ...
    <div className="mt-8 mb-12 px-4 max-w-5xl mx-auto">
      ...
      <div className="mb-4">
        ...
        <Typography variant="small" className="text-gray-500">
          Professional view of pending cities distribution.
        </Typography>
      </div>
